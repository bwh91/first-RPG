...
public class BattleManager : MonoBehaviour
{
    ...
    public Transform[] enemyPostitions;
    ...
    public void BattleStart(string[] enemiesToSpawn, bool setCannotFlee = false)
    {
        if (!battleActive)
        {
                ...
                if (GameManager.instance.playerStats[i].gameObject.activeInHierarchy)
                {
                    for (int j = 0; j < playerPrefabs.Length; j++)
                    {
                        if (playerPrefabs[j].charName == GameManager.instance.playerStats[i].charName)
                        {
                            ...
                            activeBattlers[i].armorPower = thePlayer.armrPwr;
                        }
                    }
            ...
            for (int i = 0; i < enemiesToSpawn.Length; i++)
            {
                if (enemiesToSpawn[i] != "")
                {
                    for (int j = 0; j < enemyPrefabs.Length; j++)
                    {
                        ...
                        {
                            BattleChar newEnemy = Instantiate(enemyPrefabs[j], enemyPostitions[i].position, enemyPostitions[i].rotation);
                            newEnemy.transform.parent = enemyPostitions[i];
    ...
    public void Flee()
    {
        ...
        else
        {
            ...
            {
                NextTurn();
                battleNotice.theText.text = "Couldnt escape!";
                battleNotice.Activate();
            }
        }
    }
